#!/usr/bin/env python3
"""
Import Performance Indicators from Master Table
...
# Performance Indicators data from master table
...
# Final verification
total_pis = env['performance.indicator'].search_count([('programme_id', '=', programme.id)])
...
for outcome_key, outcome in outcome_map.items():
    ...
    output_pis = env['performance.indicator'].search([
        ('programme_id', '=', programme.id),
        ('outcome_id', '=', outcome.id),
        ('indicator_type', '=', 'output')
    ])
    print(f"   • {outcome.name}:")
    print(f"     - Outcome Indicators: {len(outcome_pis)}")
    print(f"     - Output Indicators: {len(output_pis)}")
